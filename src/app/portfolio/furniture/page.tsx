...
export const metadata = { title: "Мебель на заказ — ИП Наумович" };

export default function PortfolioFurniturePage() {
  const items = facades.filter(f => f.category === "work_furniture");
  ...
  return (
    <main className="w-full bg-white pt-20 min-h-screen">
      <div className="w-full px-5 py-16 sm:px-8 lg:px-12 xl:px-16">
        ...
        <h1 className="text-4xl font-extralight tracking-tight text-gray-900 mb-12">Мебель изготовленная на заказ</h1>

        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
           {items.length === 0 ? <p className="text-gray-500">В этом разделе пока нет работ.</p> : items.map((item) => (
